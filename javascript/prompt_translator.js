(() => {
	...
	const PROMPT_RESTORE_DELAY_MS = 400;
	...
	const TABS = [
		{
			...
			negativePromptSelectors: [
				"#txt2img_neg_prompt textarea",
				"textarea#txt2img_neg_prompt",
			],
	...
	const state = {
		rows: new Map(),
		generationTranslationCache: new Map(),
	...
	function getStoredEnabled(tabOrName) {
		return (
			getStorageValueWithLegacy(tabOrName, STORAGE_KEYS.enabled, "0") === "1"
		);
	}

	function setStoredEnabled(tabOrName, enabled) {
		localStorage.setItem(
			tabStorageKey(STORAGE_KEYS.enabled, tabOrName),
			enabled ? "1" : "0",
		);
	}

	function getStoredSource(tabOrName) {
		const value = getStorageValueWithLegacy(
			tabOrName,
			STORAGE_KEYS.source,
			"Russian",
		);
		return FROM_LANGUAGES.includes(value) ? value : "Russian";
	}
	...
	function getLastDetectedSource(tabOrName) {
		const value = getStorageValueWithLegacy(
			tabOrName,
			STORAGE_KEYS.lastDetectedSource,
			"",
		);
	...
	function setLastDetectedSource(tabOrName, value) {
		if (TO_LANGUAGES.includes(value)) {
			localStorage.setItem(
				tabStorageKey(STORAGE_KEYS.lastDetectedSource, tabOrName),
				value,
			);
		}
	}
	...
	function setLastTarget(tabOrName, value) {
		if (TO_LANGUAGES.includes(value)) {
			localStorage.setItem(
				tabStorageKey(STORAGE_KEYS.lastTarget, tabOrName),
				value,
			);
		}
	}
	...
	function translationCacheKey(text, source, target) {
		return JSON.stringify([text, source, target]);
	}
	...
	async function translateTabPrompt(tab, options = {}) {
		...
		const effectiveSource = useRememberedSource
			? rememberedDetectedSource
			: source;

		if (
			!currentText.trim() ||
			(effectiveSource !== "Auto Detect" && effectiveSource === target)
		) {
			return true;
		}
		...
		try {
			const cacheKey = translationCacheKey(currentText, source, target);
			let result = state.generationTranslationCache.get(cacheKey);
			if (!result) {
				result = await callTranslateApi(currentText, effectiveSource, target);
				if (
					result &&
					result.ok !== false &&
					typeof result.translated_text === "string"
				) {
					state.generationTranslationCache.set(cacheKey, result);
				}
			}
			...
			if (
				result &&
				typeof result.detected_source === "string" &&
			...
			if (
				...
				// Keep remembered source when generation-time Auto Translate guesses the target.
				detectedSource = rememberedDetectedSource;
			}
			if (detectedSource) {
			...
			if (result && result.ok === false) {
				console.warn(
					"[prompt-translator] backend error:",
	...
	function updateVoicePreview(session, interimTranscript) {
		const spokenText = (
			session.finalTranscript + (interimTranscript || "")
		).trim();
	...
	function attachRecognitionHandlers(tab, session, recognition) {
		recognition.onresult = (event) => {
			...
			for (let i = event.resultIndex; i < event.results.length; i += 1) {
				...
				if (result.isFinal) {
					session.finalTranscript = appendVoiceChunk(
		...
		recognition.onend = () => {
			...
			session.restartTimer = window.setTimeout(() => {
				if (
					!session.keepListening ||
					state.voiceSessions.get(tab.name) !== session
				) {
					return;
				}
				...
				try {
					...
					console.warn(
						"[prompt-translator] voice input restart failed:",
						error,
					);
					finalizeVoiceSession(tab.name, session);
				}
	...
	function startVoiceInput(tab, micButton) {
		...
		if (!SpeechRecognitionCtor) {
			console.warn(
				"[prompt-translator] SpeechRecognition is not supported in this browser",
			);
			return;
		}
		...
		if (!promptArea) {
			console.warn(
				"[prompt-translator] prompt area not found for voice input:",
				tab.name,
			);
			return;
		}
	...
	function mountTranslatorRow(tab) {
		...
		const promptHost =
			promptArea.closest(".gradio-textbox") || promptArea.parentElement;
		...
		const fromSelect = createLanguageSelect(
			getStoredSource(tab),
			FROM_LANGUAGES,
		);
		...
		swapButton.style.border =
			"1px solid var(--button-secondary-border-color, var(--border-color-primary))";
		swapButton.style.borderRadius = "var(--radius-lg, 10px)";
		swapButton.style.background =
			"var(--button-secondary-background-fill, var(--input-background-fill))";
		swapButton.style.color =
			"var(--button-secondary-text-color, var(--body-text-color))";
	...
	function boot() {
		...
		for (const tab of TABS) {
			...
			setStoredSource(tab, "Russian");
			setStoredTarget(tab, "English");
			localStorage.removeItem(
				tabStorageKey(STORAGE_KEYS.lastDetectedSource, tab),
			);
